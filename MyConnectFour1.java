...
public class MyConnectFour {
    ...
    private char[][] board = new char[6][7];//size of board 
    ...
    private void playGame(char[][] board){
        ...
        while(!win){
            // player 1
            System.out.println("It's Player1 turn");//alert player
            ...
            //Determine if the game is over, not over, and player 2 turn
            else{
                //player 2
...
/*
    private void placeCounter(char player, int position){
        ...
        if(player == 'r'){
            ...
            for(int i=board.length-1; i>=0; i--){
                if(!placed){
                    if(board[i][position-1] == 'r'){//两个等于 不是一个
                        // 
                        placed = true;
                    }
 ...
 //   private void placeCounter(char player, int x, int y) {
    private void placeCounter(char player,int x){
        boolean placed = false;
        if (player == 'r') {
            for(int i=board.length-1; i>=0; i++){
                if (board[x][0] != 'r' && board[x][0] != 'y') {
                    board[x][0] = 'r';
                }else if{
                    System.out.println("This position is occupied, please input agian!");
                    System.out.println("Please re-input X position:");
